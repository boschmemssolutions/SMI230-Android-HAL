...
static inline void storage_init()
{
	...
	chmod(path,
	      0766); //notice the "umask" could mask some privilege when mkdir
...
#define BST_LOG_TAG "sensord"
		...
		switch (level) {
		case LOG_LEVEL_N:
			__android_log_print(ANDROID_LOG_FATAL, BST_LOG_TAG,
					    "%s", buffer);
			break;
		case LOG_LEVEL_E:
			__android_log_print(ANDROID_LOG_ERROR, BST_LOG_TAG,
					    "%s", buffer);
			break;
		case LOG_LEVEL_W:
			__android_log_print(ANDROID_LOG_WARN, BST_LOG_TAG, "%s",
					    buffer);
			break;
		case LOG_LEVEL_I:
			__android_log_print(ANDROID_LOG_INFO, BST_LOG_TAG, "%s",
					    buffer);
			break;
		case LOG_LEVEL_D:
			__android_log_print(ANDROID_LOG_DEBUG, BST_LOG_TAG,
					    "%s", buffer);
			break;
			     ...
			     char *info_str)
{
	if (NULL == (*p_dest_fp)) {
		...
		chmod(dest_path, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP |
					 S_IROTH | S_IWOTH);
	}
