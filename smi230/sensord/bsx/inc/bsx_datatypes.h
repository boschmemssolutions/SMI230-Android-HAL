...
#define BSX_CREATE_FIFO_LOCAL(name, type, n_dims, n_frames)                    \
	...
	bsx_fifo_data_t bsx_##name = { 0,	 (bsx_##name##_buffer),        \
				       (type),	 0U,                           \
				       (n_dims), (n_frames) };
...
#define BSX_CREATE_FIFO_GLOBAL(name, type, n_dims, n_frames)                   \
	...
	bsx_fifo_data_t bsx_##name##_g = { 0,	     (bsx_##name##_buffer_g),  \
					   (type),   0U,                       \
					   (n_dims), (n_frames) };
...
#define BSX_CREATE_FIFOS_LOCAL(name, count, type, n_dims, n_frames)             \
	...
	{                                                                       \
		bsx_size_t n_buffer =                                           \
			sizeof(bsx_data_content_t) * (n_dims) * (n_frames);     \
		for (bsx_size_t idx = 0U; idx < count; ++idx) {                 \
			bsx_##name[idx].time_stamp = 0U;                        \
			bsx_##name[idx].content_p =                             \
				(bsx_##name##_buffer)[idx];                     \
